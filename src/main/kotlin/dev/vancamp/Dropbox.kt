...
object Index {
    private val templates = PebbleTemplates().CachingClasspath()
...
object Upload {
    ...
    operator fun invoke(s3: S3): HttpHandler = {
        files(form(it)).forEach { s3[S3Key(it.filename.replace(" ", "_"))] = it.content }.run { Response(SEE_OTHER).header("location", "/") }
    }
}
